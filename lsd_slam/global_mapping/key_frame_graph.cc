...
void KeyFrameGraph::dumpMap(std::string folder)
{
    ...
    for(unsigned int i=0; i<keyframesAll.size(); i++)
    {
        ...
        cv::imwrite(buf, cv::Mat(keyframesAll[i]->height(), keyframesAll[i]->width(),
                                 CV_32F,keyframesAll[i]->image()));

        snprintf(buf, 100, "%s/var-%d.png", folder.c_str(), i);
        cv::imwrite(buf, getVarRedGreenPlot(keyframesAll[i]->idepthVar(),
                                            keyframesAll[i]->image(),keyframesAll[i]->width(),keyframesAll[i]->height()));
    }


    int i = keyframesAll.size()-1;
    Util::displayImage("VAR PREVIEW",
                       getVarRedGreenPlot(keyframesAll[i]->idepthVar(),keyframesAll[i]->image(),
    ...
    for(unsigned int i=0; i<keyframesAll.size(); i++)
    {
        ...
        usedPixels[i] = keyframesAll[i]->numPoints / (float)
                        keyframesAll[i]->numMappablePixels;
    }
    ...
    for(unsigned int i=0; i<edgesAll.size(); i++)
    {
        ...
        res(e->firstFrame->idxInKeyframes,
            e->secondFrame->idxInKeyframes) = e->meanResidual;
        resD(e->firstFrame->idxInKeyframes,
             e->secondFrame->idxInKeyframes) = e->meanResidualD;
        resP(e->firstFrame->idxInKeyframes,
             e->secondFrame->idxInKeyframes) = e->meanResidualP;
        usage(e->firstFrame->idxInKeyframes,
        ...
        distance(e->firstFrame->idxInKeyframes,
                 e->secondFrame->idxInKeyframes) = e->secondToFirst.translation().norm();
        error(e->firstFrame->idxInKeyframes,
              e->secondFrame->idxInKeyframes) = e->edge->chi2();
    }
